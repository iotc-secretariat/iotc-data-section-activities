...
Capacity_building = data.table(read.xlsx("../Data/CB_activities.xlsx", sep.names = " ", sheet = "CB_summ"))
...
Capacity_building_table = data.table(read.xlsx("../Data/CB_activities.xlsx", sep.names = " ", sheet = "CB_tab"))
...
Data_request = data.table(read.xlsx("../Data/data_request summ.xlsx", sep.names = " "))
...
## Progresss
WPDCS_progress_recomm = data.table(read.xlsx("../Data/Progress.xlsx", sep.names = " "))

PR_summary = 
  flextable(WPDCS_progress_recomm, col_keys = c("Type_recomm", "WPDCS19_REC", "Recommendations", "SC26", "Response / follow-up by the SC26", "S28", "Commission response / suggestions for consideration at WPDCS19" )) %>%
  fontsize(size = 7, part = "header")%>%
  fontsize(size = 8, part = "body")%>%
  border_inner() %>%
  border_outer(border = fp_border(width = 2)) %>%
